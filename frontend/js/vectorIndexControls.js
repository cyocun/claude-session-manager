...
export function createVectorIndexControls(deps) {
    ...
    async function refresh() {
        const [indexStatus, cached] = await Promise.all([
            invoke('get_vector_index_status'),
            invoke('is_embedding_model_cached'),
        ]);
    ...
    async function triggerBuild() {
        const cached = (await invoke('is_embedding_model_cached'));
        ...
        try {
            ...
            if (onToast)
                onToast(t('vectorIndexBuildDone'));
        }
        catch (e) {
            console.error('[vector-index] build failed', e);
            if (onToast)
                onToast(t('vectorIndexBuildFailed'));
        }
        finally {
            await refresh();
        }
