...
const searchView = createSearchView({
  ...
  projectDisplayName,
  openSession: (sessionId: string, messageIndex: number) => {
...
const fullTextSearch = createFullTextSearchController({
  ...
  setProjectFilter: (path: string | null) => setProjectFilter(path),
  resolveProjectPath: (name: string) => resolveProjectPath(name),
  invoke,
  searchView: {
...
function resolveProjectPath(displayName: string): string | null {
  const target = displayName.toLocaleLowerCase();
  const match = sessions.find(
    (s) => projectDisplayName(s.project).toLocaleLowerCase() === target
  );
  return match ? match.project : null;
}

function setProjectFilter(path: string | null): void {
  if (selectedProject === path) return;
  selectedProject = path;
  renderProjectFilterChip();
  requestRenderSessions();
  void fullTextSearch.perform((byId('search') as HTMLInputElement).value);
}

function renderProjectFilterChip(): void {
...
function renderSidebarProjectIcon(projectPath: string, uri: string | null): HTMLElement {
  const name = projectDisplayName(projectPath);
  if (uri) {
    const img = createEl('img', { className: 'project-group-favicon' }) as HTMLImageElement;
    img.src = uri;
    img.alt = name;
    return img;
  }
...
function getProjectIconUri(path: string): Promise<string | null> {
  if (iconCache.has(path)) return Promise.resolve(iconCache.get(path)!);
  const existing = iconInflight.get(path);
  if (existing) return existing;
  const p = (invoke('get_project_icon', { project: path }) as Promise<string | null>)
...
function renderProjectCard(p: ProjectInfo, iconUri: string | null): HTMLElement {
  ...
  actionsGrid.appendChild(makeIconBtn(t('newSession'), newIcon, async () => {
    ...
    setTimeout(() => fetchSessions(byId<HTMLInputElement>('showArchived').checked), 2000);
  }));
...
function showStartupView() {
  ...
  if (searchView.isActive()) fullTextSearch.clear();
...
function renderSessionItem(s: SessionSummary): HTMLElement {
  ...
  if (isActive) activeSessionItemEl = item;
  ...
  item.addEventListener('mouseenter', () => {
    if (!item.classList.contains('session-item-active')) item.style.background = 'var(--item-hover)';
  ...
  item.addEventListener('mouseleave', () => {
    if (!item.classList.contains('session-item-active')) item.style.background = defaultBg;
    schedulePreviewHide();
  });
  return item;
}

function setSessionItemActiveState(item: HTMLElement, isActive: boolean): void {
  if (isActive) {
...
function findSessionItemById(sessionId: string): HTMLElement | null {
  const listEl = byId('sessionList') as HTMLElement | null;
  if (!listEl) return null;
  const items = listEl.querySelectorAll('.session-item');
  for (let i = 0; i < items.length; i++) {
    const item = items[i] as HTMLElement;
    if (item.getAttribute('data-id') === sessionId) return item;
  }
  return null;
}

function syncActiveSessionItem(nextItem: HTMLElement | null = null): void {
...
function matchesSessionSearch(s: SessionSummary, searchLower: string): boolean {
  ...
  return (
    (s.firstDisplay || '').toLowerCase().includes(searchLower) ||
...
function getFilteredSessions(): SessionSummary[] {
  const searchLower = (byId('search') as HTMLInputElement).value.toLowerCase();
...
function groupSessionsByProject(filtered: SessionSummary[]): ProjectGroup[] {
...
function renderProjectGroup(g: ProjectGroup, groups: ProjectGroup[]): HTMLElement {
  ...
  const startBtn = createEl('button', {
    ...
    onClick: async (e: Event) => {
      ...
      if (!result?.ok) actions.showToast(t('toastError') + (result?.error || ''));
      else setTimeout(() => fetchSessions(byId<HTMLInputElement>('showArchived').checked), 2000);
    }
  });
  ...
  if (selectedProject === g.path) filterBtn.classList.add('active');
  ...
  if (!iconCache.has(g.path)) {
    getProjectIconUri(g.path).then((uri) => {
      if (uri) icon.replaceChildren(renderSidebarProjectIcon(g.path, uri));
    });
  }
...
async function getDetailWithCache(sessionId: string): Promise<SessionDetail | null> {
...
function renderDetailHeader(sessionId: string, detail: SessionDetail, headerEl: HTMLElement): void {
...
function renderMsgDesc(desc: MsgDesc, resultMap: Record<string, any>): HTMLElement[] {
  ...
  if (isRecap && hasText) {
    ...
    const labelChildren: Node[] = [icon, createEl('span', { textContent: 'Recap' })];
...
async function showDetail(sessionId: string) {
  const pane = byId('detailPane') as HTMLElement;
  const headerEl = byId('detailHeader') as HTMLElement;
...
function toggleTerminalForSelected(): void {
...
async function refreshSessionsAction(): Promise<void> {
  await fetchSessions(byId<HTMLInputElement>('showArchived').checked);
}

async function archiveCurrentSessionAction(): Promise<void> {
  if (!selectedSession) return;
  await actions.archiveSingle(selectedSession);
}

function focusSessionByIndex(idx: number): void {
...
byId<HTMLInputElement>('search').addEventListener('keydown', (e: KeyboardEvent) => {
...
Promise.all([fetchSessions(), fetchProjects(), fetchSettings()]).then(() => {
  ...
  showStartupView();
  initUpdater({ showToast: (m: string) => actions.showToast(m) });
});
