...
export function hasSessionIdentityOrderChanged(
  prev: SessionSummary[],
  next: SessionSummary[],
  sevenDaysMs: number,
  now = Date.now(),
...
type PatchDeps = {
  ...
  formatDateTitle: (timestamp: number) => string;
  isUpdatedSession: (session: SessionSummary) => boolean;
};

function updateMetaBadge(metaDiv: HTMLElement, selector: string, className: string, text: string): void {
  let el = metaDiv.querySelector(selector) as HTMLElement | null;
  if (!text) {
    el?.remove();
    return;
  }
...
function ensureUpdatedDot(item: HTMLElement, shouldShow: boolean): void {
  const row = item.querySelector('.session-row') as HTMLElement | null;
  if (!row) return;
  const existing = row.querySelector('.session-updated-dot') as HTMLElement | null;
  if (shouldShow) {
    if (!existing) {
...
export function patchSessionListItems(deps: PatchDeps): boolean {
  ...
  const items = Array.from(root.querySelectorAll('.session-item')) as HTMLElement[];
  if (items.length !== sessions.length) return false;

  for (let i = 0; i < sessions.length; i++) {
    const session = sessions[i];
    const item = items[i];
    if (item.dataset.id !== session.sessionId) return false;

    const first = item.querySelector('.session-first') as HTMLElement | null;
    if (first) first.textContent = session.firstDisplay;

    const last = item.querySelector('.session-last') as HTMLElement | null;
    const hasLast = Boolean(session.lastDisplay && session.lastDisplay !== session.firstDisplay);
    if (hasLast) {
      if (last) last.textContent = session.lastDisplay || '';
      else {
        const textDiv = item.querySelector('.session-text') as HTMLElement | null;
        if (textDiv) {
    ...
    const metaTime = item.querySelector('.session-meta-time') as HTMLElement | null;
    if (metaTime) metaTime.textContent = formatTimeAgo(session.lastTimestamp);
    const metaCount = item.querySelector('.session-meta-count') as HTMLElement | null;
    ...
    if (checkbox) checkbox.checked = selectedIds.has(session.sessionId);
