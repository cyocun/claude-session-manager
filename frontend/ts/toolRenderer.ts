...
function guessLang(filePath?: string): string | null {
...
function highlightCode(text: string, lang: string | null): string | null {
...
function makeHighlightedPre(text: string, lang: string | null): HTMLElement {
  const pre = document.createElement('pre');
  const code = document.createElement('code');
  code.className = 'hljs' + (lang ? ' language-' + lang : '');
  const highlighted = highlightCode(text, lang);
  if (highlighted) code.innerHTML = DOMPurify.sanitize(highlighted);
...
function guessLangFromKey(key: string): string | null {
  const k = key.toLowerCase();
  if (/(^|_)(function|script|code|js|javascript|expression)($|_)/.test(k)) return 'javascript';
  if (/(^|_)(ts|typescript)($|_)/.test(k)) return 'typescript';
  if (/sql|query/.test(k)) return 'sql';
  if (/html|markup/.test(k)) return 'xml';
  if (/css|style/.test(k)) return 'css';
  if (/(^|_)json($|_)/.test(k)) return 'json';
  if (/bash|shell|cmd|command/.test(k)) return 'bash';
  if (/(^|_)(yaml|yml)($|_)/.test(k)) return 'yaml';
  return null;
}

function looksLikeCodeValue(v: string): boolean {
  return v.includes('\n') || v.length > 160;
}

function makeStructuredInputBody(input: unknown): HTMLElement {
  if (!input || typeof input !== 'object' || Array.isArray(input)) {
    return makeHighlightedPre(JSON.stringify(input, null, 2), 'json');
  }
  const obj = input as Record<string, unknown>;
  const keys = Object.keys(obj);
  const hasCodeValue = keys.some((k) => typeof obj[k] === 'string' && looksLikeCodeValue(obj[k] as string));
  if (!hasCodeValue) {
    return makeHighlightedPre(JSON.stringify(input, null, 2), 'json');
  }
...
function prettyPrintFencedJson(text: string): string {
...
export function renderToolBlocks(
  ...
  tools.filter((t) => t.name === '_result').forEach((t) => { resultMap[t.id] = t as ToolResult; });
  ...
  tools.filter((t) => t.name !== '_result').forEach((tool) => {
    ...
    if (tool.name === 'Bash') {
      ...
      if (tool.command) bodyEl = makeHighlightedPre('$ ' + tool.command, 'bash');
      ...
      summary = tool.name;
      if (tool.input !== undefined) bodyEl = makeStructuredInputBody(tool.input);
    }
    ...
    if (hasBody) block.appendChild(body);
    els.push(block);
  });
  return els;
}
